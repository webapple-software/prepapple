...
import { User, Shield, GraduationCap, Plus, Trash2, LogOut, BookOpen, Key, Eye, EyeOff, UploadCloud, RefreshCw, X, Check, CheckCircle2, AlertTriangle, FileSpreadsheet, BarChart2, Layers, Activity, Download } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const handleToggleSubscription = async (id: number) => {
    try {
      const response = await fetch(`http://localhost:5000/api/admin/students/${id}/subscription`, {
        method: 'PUT'
      });
      if (!response.ok) throw new Error('Failed to toggle student subscription');
      const data = await response.json();
      
      setStudents(prev => prev.map(s => s.id === id ? { ...s, is_subscribed: data.isSubscribed ? 1 : 0 } : s));
      Swal.fire({
        title: 'Success!',
  ...
  const handleToggleActive = async (id: number) => {
    try {
      const response = await fetch(`http://localhost:5000/api/admin/students/${id}/active`, {
        method: 'PUT'
      });
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex font-sans select-none">
      ...
      {/* Sidebar Navigation */}
      <aside className="w-72 bg-[#0f294a] text-white flex flex-col justify-between border-r border-blue-950 flex-shrink-0 select-none">
        <div>
          {/* Logo Brand */}
          <div className="px-6 py-6 border-b border-blue-950 flex items-center gap-3">
            <div className="bg-blue-600/30 p-2 rounded-xl border border-blue-400/30 text-white flex items-center justify-center">
              <Shield className="w-5.5 h-5.5" />
            </div>
            <div>
              <h1 className="font-extrabold text-white text-base md:text-lg tracking-tight uppercase">
                PrepApple
              </h1>
          ...
          <nav className="mt-6 px-4 space-y-1.5">
            ...
            <button
              onClick={() => setActiveTab('import_questions')}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${
        ...
        <div className="p-4 border-t border-blue-950 space-y-1.5">
          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-wider text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-all cursor-pointer"
          >
            ...
            <div className="lg:col-span-1">
              <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
                ...
                <form onSubmit={handleAddStudent} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Target Course / Category</label>
                    <select
                      value={sCourse}
                      onChange={(e) => setSCourse(e.target.value)}
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-semibold text-sm text-gray-700 bg-white cursor-pointer"
                    >
                      {categories.map(c => (
                            ...
                            <td className="p-4">
                              <button
                                ...
                                className={`px-2.5 py-1 text-[9px] font-black rounded-lg border transition-all cursor-pointer ${
                                  student.is_subscribed === 1
                                    ? 'bg-emerald-50 text-emerald-700 border-emerald-250 hover:bg-emerald-100 shadow-sm'
                                    : 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200 shadow-sm'
                                }`}
                                title="Click to toggle subscription status"
                              >
                                {student.is_subscribed === 1 ? 'SUBSCRIBED' : 'FREE ACCESS'}
                              </button>
                            </td>
                            <td className="p-4">
                              <button
                              ...
                              >
                                {student.is_active !== 0 ? 'ACTIVE' : 'INACTIVE'}
                              </button>
                            </td>
        ...
        {activeTab === 'categories' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
                <h3 className="text-lg font-black text-gray-800 mb-5 flex items-center gap-1.5">
                  <Plus className="w-5 h-5 text-blue-600" /> Create Category
                </h3>

                <form onSubmit={handleCreateCategory} className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Category ID (Unique)</label>
                    <input
                      type="text"
                      required
                      placeholder="e.g. ssc, banking, jee"
                      value={newCatId}
                      onChange={(e) => setNewCatId(e.target.value)}
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-semibold text-sm text-gray-800"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Category Title (Display Name)</label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Theme Icon</label>
                    <select
                      value={newCatIcon}
                      onChange={(e) => setNewCatIcon(e.target.value)}
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-semibold text-sm text-gray-700 bg-white cursor-pointer"
                    >
                      <option value="Calculator">Calculator (Math / Tech)</option>
                      <option value="Stethoscope">Stethoscope (Medical)</option>
                      <option value="Landmark">Landmark (Govt Jobs)</option>
                      <option value="Train">Train (Railways)</option>
                      <option value="Crosshair">Crosshair (Defence)</option>
                      <option value="BookOpen">BookOpen (Teaching)</option>
                      <option value="ShieldCheck">ShieldCheck (Police / Security)</option>
                      <option value="GraduationCap">GraduationCap (General Academic)</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Theme Color Accent</label>
                    <select
                      value={newCatColor}
                  ...
                  <button
                    type="submit"
                    className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs rounded-xl uppercase tracking-wider shadow transition-colors cursor-pointer mt-4"
                  >
                    Save Exam Category
                  </button>
                </form>
            ...
            <div className="lg:col-span-2">
              <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
                <h3 className="text-lg font-black text-gray-800 mb-5 flex items-center gap-1.5">
                  <Layers className="w-5.5 h-5.5 text-blue-600" /> Active Categories Directory
                </h3>

                {categories.length === 0 ? (
                  <div className="border border-dashed border-gray-200 rounded-2xl p-12 text-center text-gray-400">
                    <Layers className="w-10 h-10 mx-auto text-gray-300 mb-2" />
                    <p className="text-sm font-semibold">No dynamic categories configured yet.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {categories.map((cat) => {
                      ...
                      const selectedColorClass = colorMap[cat.color as string] || colorMap.blue;

                      return (
                        <div key={cat.id} className={`border rounded-2xl p-5 shadow-sm transition-all hover:shadow-md flex items-start justify-between ${selectedColorClass}`}>
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="font-extrabold text-sm uppercase tracking-wide">{cat.title}</span>
                              <span className="text-[9px] bg-white/80 border px-1.5 py-0.5 rounded font-black font-mono">{cat.id}</span>
                            </div>
                            <p className="text-[10px] opacity-75 font-semibold mt-1">
                              Icon theme: <span className="font-bold">{cat.icon}</span>
                            </p>
                            <p className="text-[10px] opacity-75 font-semibold">
                              Created at: <span className="font-bold">{cat.created_at ? new Date(cat.created_at).toLocaleDateString() : 'System Seed'}</span>
                            </p>
                          </div>
                          <button
                            type="button"
                            onClick={() => handleDeleteCategory(cat.id)}
                            className="p-2 hover:bg-red-100/60 rounded-xl text-red-600 transition-colors cursor-pointer"
            ...
            <form onSubmit={handleUploadSet} className="p-6 space-y-4 max-h-[calc(100vh-160px)] overflow-y-auto">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 text-xs">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2.5">
                  ...
                  <a
                    href="http://localhost:5000/api/import-questions/template"
