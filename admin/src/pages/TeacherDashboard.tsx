...
export default function TeacherDashboard({ isEmbedded = false }: { isEmbedded?: boolean }) {
  ...
  const openAssignModal = async (testId: number) => {
    try {
      ...
      const assignmentsRes = await fetch(`http://localhost:5000/api/tests/${testId}/assignments`);
  ...
  const handleSaveAssignments = async (testId: number) => {
    try {
      setLoadingAssignments(true);
      const response = await fetch(`http://localhost:5000/api/tests/${testId}/assignments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-100 pb-5 mb-6">
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase border ${
                        selectedTest.test_type === 'quiz' || selectedTest.test_type === 'daily'
                          ? 'bg-[#10b981]/10 text-[#10b981] border-[#10b981]/30'
                          : selectedTest.test_type === 'sectional' || selectedTest.test_type === 'topic'
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <label className="flex items-center gap-2 text-xs font-bold text-gray-700 cursor-pointer select-none bg-slate-50 border border-gray-200 px-3 py-2 rounded-xl">
                      ...
                      />
                      <span className={selectedTest.test_type === 'quiz' || selectedTest.test_type === 'daily' ? 'opacity-50' : ''}>Free Test</span>
                    </label>
                    <select
                    ...
                    >
                      ...
                      <option value="daily">Daily Practice Test</option>
                      <option value="sectional">Sectional Mock Test</option>
                      <option value="topic">Topic Practice Test</option>
                    </select>
                    <button
            ...
            <form onSubmit={handleCreateTest} className="space-y-4 max-h-[calc(100vh-165px)] overflow-y-auto pr-1">
              ...
              <div>
                ...
                >
                  ...
                  <option value="quiz">Practice Quiz (Always Free)</option>
                  <option value="daily">Daily Practice Test (Always Free)</option>
                  <option value="sectional">Sectional Mock Test</option>
                  <option value="topic">Topic Practice Test</option>
                </select>
              </div>
