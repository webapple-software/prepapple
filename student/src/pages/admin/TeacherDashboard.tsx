...
import { Plus, Trash2, Edit, Eye, X, BookOpen, Clock, HelpCircle, FileText, CheckCircle, UploadCloud, RefreshCw, AlertTriangle, FileSpreadsheet, Users, Search, Zap } from 'lucide-react';
import examPatternsData from '../../data/exam_patterns.json';
...
export default function TeacherDashboard({ isEmbedded = false }: { isEmbedded?: boolean }) {
  // Tests List
  const [tests, setTests] = useState<Test[]>([]);
  const [categories, setCategories] = useState<any[]>([]);
  const [selectedTest, setSelectedTest] = useState<Test | null>(null);

  // Search & Filter state for left sidebar tests
  ...
  const [newTestType, setNewTestType] = useState<string>('mock');
  ...
  const handleToggleType = async (test: Test, type: string) => {
    try {
      const isFreeVal = (type === 'quiz' || type === 'daily') ? 1 : test.is_free;
      const response = await fetch(`/api/tests/${test.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      fetchTests();
      if (selectedTest?.id === test.id) {
        setSelectedTest(prev => prev ? { ...prev, test_type: type, is_free: isFreeVal } : null);
      }
      Swal.fire({
        title: 'Updated!',
        text: `Mock test converted to ${type.toUpperCase()}.`,
        icon: 'success',
        confirmButtonColor: '#10b981'
      });
    } catch (error: any) {
  ...
  return (
    <div className={isEmbedded ? "max-w-7xl mx-auto" : "max-w-7xl mx-auto px-4 py-8"}>
      {/* Dashboard Header */}
      {isEmbedded ? (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6 pb-4 border-b border-gray-150">
          <p className="text-gray-500 text-xs font-semibold">Create tests, manage questions, and review student attempt performance.</p>
          <button
            onClick={() => setShowCreateModal(true)}
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-xl transition-all shadow-md hover:shadow-lg cursor-pointer text-xs uppercase tracking-wider"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="flex flex-col gap-3">
            <h2 className="text-lg font-black text-slate-800 uppercase tracking-tight">Active Mock Tests</h2>

            {/* Search and Category Filters for Sidebar */}
            <div className="flex flex-col gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 border border-slate-205 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-semibold text-xs text-gray-800 bg-slate-50/50"
                />
              </div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-slate-205 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-semibold text-xs text-gray-700 bg-white cursor-pointer"
              >
          ...
          {loadingTests ? (
            <div className="space-y-3">
              {[1, 2, 3].map(n => (
                <div key={n} className="bg-white border border-slate-100 rounded-2xl h-24 animate-pulse"></div>
              ))}
            </div>
          ) : (() => {
            const filteredTests = tests.filter(test => {
              const matchesQuery = (test.name || '').toLowerCase().includes(testSearchQuery.toLowerCase());
              if (!matchesQuery) return false;
              if (testFilterCategory !== 'ALL') {
                if ((test.category || 'JEE').toUpperCase() !== testFilterCategory.toUpperCase()) return false;
              }
              return true;
            });
            ...
            if (filteredTests.length === 0) {
              return (
                <div className="bg-white border border-dashed border-slate-200 rounded-3xl p-8 text-center text-slate-400 select-none">
                  <BookOpen className="w-10 h-10 mx-auto text-slate-300 mb-2" />
                  <p className="text-xs font-black uppercase tracking-tight text-slate-700">No Tests Found</p>
                  <p className="text-[10px] text-slate-400 mt-1 font-semibold">Try matching another query or create a new mock test.</p>
                </div>
              );
            }
              ...
              <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
                {filteredTests.map(test => {
                  ...
                  return (
                    <div
                      key={test.id}
                      className={`bg-white border rounded-2xl p-4 transition-all shadow-sm flex flex-col justify-between cursor-pointer group ${
                        isSelected 
                          ? 'border-blue-600 ring-2 ring-blue-50/50 bg-blue-50/5' 
                    ...
                    >
                      <div className="flex justify-between items-start gap-2">
                        <h3 className={`font-bold text-slate-800 text-xs line-clamp-2 transition-colors ${isSelected ? 'text-blue-700' : 'group-hover:text-blue-650'}`}>
                          {test.name}
                        </h3>
                        <button
                          onClick={(e) => {
                          ...
                          className="text-slate-300 hover:text-red-600 p-1 rounded-lg hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100 animate-fade-in"
                      ...
                      <div className="flex justify-between items-center mt-4 text-[10px] text-slate-400 font-semibold border-t border-slate-50 pt-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase border select-none ${
                            test.category === 'NEET' 
                          ...
                          <span className="flex items-center gap-0.5"><Clock className="w-3 h-3 text-slate-400" />{test.duration}m</span>
                          <span className="flex items-center gap-0.5"><HelpCircle className="w-3 h-3 text-slate-400" />{test.question_count} Qs</span>
                        </div>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className={`px-2.5 py-0.5 rounded-full text-[9px] font-black border transition-all uppercase select-none ${
                            test.is_published === 1
      ...
      <div className="lg:col-span-2">
          {selectedTest ? (
            <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm min-h-[500px] flex flex-col">
              {/* Redesigned Test Title & Description */}
                <div className="flex flex-col gap-3 mb-5">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight leading-snug">{selectedTest.name}</h2>
                    <span className="bg-blue-50 text-blue-700 border border-blue-200 text-[8px] font-black px-2 py-0.5 rounded-lg uppercase tracking-wider">
                      {selectedTest.category || 'JEE'}
                    </span>
                    <span className="bg-indigo-50 text-indigo-700 border border-indigo-200 text-[8px] font-black px-2 py-0.5 rounded-lg uppercase tracking-wider">
                      {selectedTest.test_type || 'MOCK'}
                    </span>
                  </div>
                  <p className="text-slate-400 text-xs font-semibold">
                    Duration: <span className="text-slate-700 font-bold">{selectedTest.duration} mins</span> • Questions:{' '}
                    <span className="text-slate-700 font-bold">{selectedTest.questions?.length || 0}</span>
                  </p>
                </div>

                {/* Redesigned Control Dashboard Panel */}
                <div className="bg-slate-50 border border-slate-205 rounded-3xl p-5 mb-6 flex flex-col xl:flex-row xl:items-center justify-between gap-5 select-none">
                  {/* Settings Switches */}
                  <div className="flex flex-wrap items-center gap-4">
                    {/* Randomize Order Toggle */}
                    <label className="inline-flex items-center gap-2 cursor-pointer bg-white border border-slate-200 px-3.5 py-2 rounded-xl hover:shadow-sm transition-all">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-xs font-extrabold text-slate-700 uppercase tracking-wide">Shuffle Qs</span>
                    </label>

                    {/* Free Test Toggle */}
                    <label className={`inline-flex items-center gap-2 cursor-pointer bg-white border border-slate-200 px-3.5 py-2 rounded-xl hover:shadow-sm transition-all ${
                      selectedTest.test_type === 'quiz' || selectedTest.test_type === 'daily' ? 'opacity-40 cursor-not-allowed' : ''
                    }`}>
                      <input
                        type="checkbox"
                        checked={selectedTest.is_free === 1}
                        disabled={selectedTest.test_type === 'quiz' || selectedTest.test_type === 'daily'}
                      ...
                      />
                      <span className="text-xs font-extrabold text-slate-700 uppercase tracking-wide">Free Access</span>
                    </label>
                    ...
                    <div className="flex items-center gap-2 bg-white border border-slate-200 px-3.5 py-1.5 rounded-xl">
                      ...
                      <select
                        ...
                        onChange={(e) => handleToggleType(selectedTest, e.target.value)}
                  ...
                  <div className="flex flex-wrap items-center gap-2.5">
                    <button
                      ...
                      className={`px-4.5 py-3 text-xs font-black uppercase tracking-wider rounded-xl transition-all shadow-sm hover:shadow cursor-pointer ${
                        selectedTest.is_published === 1
                          ? 'bg-emerald-605 bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-500/10'
                          : 'bg-amber-505 bg-amber-500 hover:bg-amber-600 text-white shadow-amber-500/10'
                      }`}
                    >
                      {selectedTest.is_published === 1 ? 'Unpublish' : 'Publish'}
                    </button>

                    {selectedTest.test_type !== 'quiz' && (
                      <button
                        onClick={() => openAssignModal(selectedTest.id)}
                        className="px-4.5 py-3 text-xs font-black uppercase tracking-wider bg-blue-600 hover:bg-blue-700 text-white rounded-xl shadow-sm hover:shadow transition-all flex items-center gap-1.5 cursor-pointer"
                      >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl border border-gray-100 animate-slide-up">
            ...
            <form onSubmit={handleCreateTest} className="space-y-4 max-h-[calc(100vh-165px)] overflow-y-auto pr-1">
              ...
              {/* Exam Pattern Auto-fill */}
              <div className="bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-100 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  <Zap className="w-4 h-4 text-indigo-600" />
                  <span className="text-xs font-black text-indigo-700 uppercase tracking-wider">Smart Pattern Auto-fill</span>
                </div>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (code) {
                      const pattern = (examPatternsData as any).exam_patterns.find((p: any) => p.exam_code === code);
                      if (pattern) {
                        // Calculate duration from typical exam timing (use 120 mins default if not in data, or use exam_details for duration)
                        const durationMap: Record<string, number> = {
                          'UP_POLICE_CONSTABLE': 120, 'UP_POLICE_SI': 120,
                          'MAHARASHTRA_POLICE_CONSTABLE': 90, 'BIHAR_POLICE_CONSTABLE': 120,
                          'DELHI_POLICE_CONSTABLE': 90, 'RAJASTHAN_POLICE_CONSTABLE': 120,
                          'JEE_MAIN': 180, 'NEET_UG': 200,
                          'MHT_CET_PCM': 180, 'MHT_CET_PCB': 180,
                          'SSC_CGL_TIER1': 60, 'SSC_CPO_PAPER1': 120, 'SSC_JE_PAPER1': 120,
                          'SSC_GD_CONSTABLE': 60, 'SSC_CHSL_TIER1': 60, 'SSC_MTS': 90,
                          'CTET_PAPER1': 150, 'CTET_PAPER2': 150,
                          'DSSSB_TGT': 120, 'BIHAR_TEACHER_BPSC_TRE': 150,
                          'NDA': 150, 'CDS_IMA_INA_AFA': 120, 'AFCAT': 120,
                        };
                        setNewTestDuration(String(durationMap[code] || 120));
                        // Use marks from first section or total/questions
                        const firstSection = pattern.sections?.[0];
                        const marksPerQ = firstSection?.marks_per_question || 1;
                        setNewTestMarks(String(marksPerQ));
                        const neg = pattern.negative_marking ?? 0;
                        setNewTestNegMarks(neg > 0 ? String(-neg) : '0');
                        // Also set category based on exam
                        const catMap: Record<string, string> = {
                          'UP_POLICE_CONSTABLE': 'UP POLICE CONSTABLE', 'UP_POLICE_SI': 'UP SI',
                          'MAHARASHTRA_POLICE_CONSTABLE': 'MAHARASHTRA POLICE', 'BIHAR_POLICE_CONSTABLE': 'BIHAR POLICE',
                          'DELHI_POLICE_CONSTABLE': 'DELHI POLICE', 'RAJASTHAN_POLICE_CONSTABLE': 'RAJASTHAN POLICE',
                          'JEE_MAIN': 'JEE', 'NEET_UG': 'NEET',
                          'MHT_CET_PCM': 'MHT PCM', 'MHT_CET_PCB': 'MHT PCB',
                          'SSC_CGL_TIER1': 'SSC CGL', 'SSC_CPO_PAPER1': 'SSC CPO', 'SSC_JE_PAPER1': 'SSC JE',
                          'SSC_GD_CONSTABLE': 'SSC GD', 'SSC_CHSL_TIER1': 'SSC CHSL', 'SSC_MTS': 'SSC MTS',
                          'CTET_PAPER1': 'CTET', 'CTET_PAPER2': 'CTET',
                          'DSSSB_TGT': 'UPTET', 'BIHAR_TEACHER_BPSC_TRE': 'UPTET',
                          'NDA': 'NDA', 'CDS_IMA_INA_AFA': 'CDS', 'AFCAT': 'AFCAT',
                        };
                        if (catMap[code]) setNewTestCategory(catMap[code]);
                      }
                    }
                  }}
                  className="w-full px-3 py-2.5 border border-indigo-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white font-semibold text-sm text-gray-800 cursor-pointer"
                >
                  <option value="">— Select Exam Pattern to Auto-fill —</option>
                  <optgroup label="Defence">
                    {(examPatternsData as any).exam_patterns.filter((p: any) => ['NDA','CDS_IMA_INA_AFA','AFCAT'].includes(p.exam_code)).map((p: any) => (
                      <option key={p.exam_code} value={p.exam_code}>{p.exam_name}</option>
                    ))}
                  </optgroup>
                  <optgroup label="MHT CET">
                    {(examPatternsData as any).exam_patterns.filter((p: any) => p.exam_code.startsWith('MHT')).map((p: any) => (
                      <option key={p.exam_code} value={p.exam_code}>{p.exam_name}</option>
                    ))}
                  </optgroup>
                  <optgroup label="Police">
                    {(examPatternsData as any).exam_patterns.filter((p: any) => p.exam_code.includes('POLICE')).map((p: any) => (
                      <option key={p.exam_code} value={p.exam_code}>{p.exam_name}</option>
                    ))}
                  </optgroup>
                  <optgroup label="SSC">
                    {(examPatternsData as any).exam_patterns.filter((p: any) => p.exam_code.startsWith('SSC')).map((p: any) => (
                      <option key={p.exam_code} value={p.exam_code}>{p.exam_name}</option>
                    ))}
                  </optgroup>
                  <optgroup label="Teaching">
                    {(examPatternsData as any).exam_patterns.filter((p: any) => ['CTET_PAPER1','CTET_PAPER2','DSSSB_TGT','BIHAR_TEACHER_BPSC_TRE'].includes(p.exam_code)).map((p: any) => (
                      <option key={p.exam_code} value={p.exam_code}>{p.exam_name}</option>
                    ))}
                  </optgroup>
                  <optgroup label="Engineering / Medical">
                    {(examPatternsData as any).exam_patterns.filter((p: any) => ['JEE_MAIN','NEET_UG'].includes(p.exam_code)).map((p: any) => (
                      <option key={p.exam_code} value={p.exam_code}>{p.exam_name}</option>
                    ))}
                  </optgroup>
                </select>
                ...
                {/* Pattern Preview */}
                {selectedPatternCode && (() => {
                  const pattern = (examPatternsData as any).exam_patterns.find((p: any) => p.exam_code === selectedPatternCode);
                  if (!pattern) return null;
                  return (
                    <div className="mt-3 bg-white rounded-xl border border-indigo-100 p-3 space-y-2">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-500 font-semibold">Total Questions:</span>
                        <span className="font-black text-indigo-700">{pattern.total_questions}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-500 font-semibold">Total Marks:</span>
                        <span className="font-black text-indigo-700">{pattern.total_marks}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-500 font-semibold">Negative Marking:</span>
                        <span className="font-black text-red-600">{pattern.negative_marking ?? 0}</span>
                      </div>
                      <div className="border-t border-gray-100 pt-2 mt-2">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1.5">Sections:</p>
                        <div className="space-y-1">
                          {pattern.sections?.map((sec: any, i: number) => (
                            <div key={i} className="flex justify-between items-center text-[11px]">
                              <span className="text-gray-700 font-semibold truncate max-w-[180px]">{sec.subject_name}</span>
                              <span className="text-indigo-600 font-black ml-2 shrink-0">{sec.question_count}Q</span>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. PrepApple Mock Test 01"
                  value={newTestName}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setNewTestType(val);
                    if (val === 'quiz' || val === 'daily') {
                      setNewTestIsFree(true);
                    }
                  }}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white font-semibold text-sm text-gray-800 cursor-pointer"
                >
                  <option value="mock">Full Mock Test (30+ Questions)</option>
                  <option value="quiz">Practice Quiz (Under 30 Questions, Always Free)</option>
                </select>
              </div>
